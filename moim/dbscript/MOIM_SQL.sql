...
DROP TABLE DEBATE CASCADE CONSTRAINTS;
DROP TABLE DEBATE_COMMENT CASCADE CONSTRAINTS;
...
(
    ...
    USER_NN VARCHAR2(100) NOT NULL UNIQUE,
...
ALTER TABLE TB_TEAM
    ADD CONSTRAINT FK_TB_TEAM_FIELD_NUM FOREIGN KEY (FIELD_NUM)
        REFERENCES TB_FIELD (FIELD_NUM) ON DELETE CASCADE;
...
ALTER TABLE TB_TEAM_MEMBER
    ADD CONSTRAINT FK_TB_TEAM_MEMBER_TEAM_NUM FOREIGN KEY (TEAM_NUM)
        REFERENCES TB_TEAM (TEAM_NUM) ON DELETE CASCADE;
...
(
    ...
    team_num number constraint file_team_num_fk references TB_TEAM(team_num) ON DELETE CASCADE, 
    file_uploader number constraint file_uploader_fk references USER_INFO(user_no) ON DELETE CASCADE,
...
(
    ...
    team_num number constraint dailyteam_num_nn not null references TB_TEAM(team_num) ON DELETE CASCADE,
    daily_writer number constraint daily_writer_nn not null references USER_INFO(user_no) ON DELETE CASCADE,
...
(
    ...
    team_num number constraint calteam_num_nn not null references TB_TEAM(team_num) ON DELETE CASCADE,
    cal_writer number constraint cal_writer_nn not null references USER_INFO(user_no) ON DELETE CASCADE,
...
(
    ...
    team_num number constraint tnteam_num_nn not null references TB_TEAM(team_num) ON DELETE CASCADE,
    tn_writer number constraint tn_writer_nn not null references USER_INFO(user_no) ON DELETE CASCADE,
...
CREATE TABLE REVIEW(
    ...
    TEAM_NAME VARCHAR2(200) CONSTRAINT REVIEW_TEAM_NAME_FK REFERENCES TB_TEAM(TEAM_NAME) ON DELETE CASCADE,
    FIELD_NAME VARCHAR2(100) CONSTRAINT REVIEW_FIELD_NN NOT NULL,
...
CREATE TABLE NOTICE(
    ...
    USER_NO NUMBER CONSTRAINT NOTICE_USER_NO_FK REFERENCES USER_INFO(USER_NO) ON DELETE CASCADE, 
...
CREATE TABLE BOARD(
    ...
    BOARD_CATEGORY_NO NUMBER CONSTRAINT BOARD_CATEGORY_NO_FK REFERENCES BOARD_CATEGORY(BOARD_CATEGORY_NO) ON DELETE CASCADE, 
    USER_NO NUMBER CONSTRAINT BOARD_USER_NO_FK REFERENCES USER_INFO(USER_NO) ON DELETE CASCADE, 
...
CREATE TABLE REPLY(
    ...
    BOARD_NO NUMBER CONSTRAINT BOARD_NO_FK REFERENCES BOARD(BOARD_NO) ON DELETE CASCADE, 
    USER_NO NUMBER CONSTRAINT USER_NO_FK REFERENCES USER_INFO(USER_NO) ON DELETE CASCADE, 
...
(
    ...
    TEAM_MEMBER_NO NUMBER NOT NULL, 
    TEAM_NUM NUMBER NOT NULL, 
    ALERT_CATE VARCHAR2(50) NOT NULL,
...
COMMENT ON COLUMN TB_ALERT.ALERT_CATE IS '알람 분류';
...
ALTER TABLE TB_ALERT
    ADD CONSTRAINT FK_TB_ALERT_TEAM_MEMBER_NO FOREIGN KEY (TEAM_MEMBER_NO)
        REFERENCES TB_TEAM_MEMBER (TEAM_MEMBER_NO) ON DELETE CASCADE;

ALTER TABLE TB_ALERT
    ADD CONSTRAINT FK_TB_ALERT_TEAM_NUM FOREIGN KEY (TEAM_NUM)
        REFERENCES TB_TEAM (TEAM_NUM) ON DELETE CASCADE;

-- DEBATE Table Create SQL
CREATE TABLE DEBATE
(
    DEBATE_NUM NUMBER NOT NULL, 
    USER_NO NUMBER NOT NULL, 
    TEAM_NUM NUMBER NOT NULL, 
    DEBATE_TITLE VARCHAR2(200) NOT NULL,
    DEBATE_CONTENT VARCHAR2(2000) NOT NULL,
    DEBATE_DATE DATE DEFAULT SYSDATE NOT NULL,
    DEBATE_COMMENT_COUNT NUMBER DEFAULT 0 NOT NULL, 
    CONSTRAINT PK_DEBATE PRIMARY KEY (DEBATE_NUM)
);
COMMENT ON TABLE DEBATE IS '토론';
COMMENT ON COLUMN DEBATE.DEBATE_NUM IS '토론 번호';
COMMENT ON COLUMN DEBATE.USER_NO IS '회원 번호';
COMMENT ON COLUMN DEBATE.TEAM_NUM IS '팀 번호';
COMMENT ON COLUMN DEBATE.DEBATE_TITLE IS '토론 제목';
COMMENT ON COLUMN DEBATE.DEBATE_CONTENT IS '토론 내용';
COMMENT ON COLUMN DEBATE.DEBATE_DATE IS '토론 날짜';
COMMENT ON COLUMN DEBATE.DEBATE_COMMENT_COUNT IS '댓글 갯수';

ALTER TABLE DEBATE
    ADD CONSTRAINT FK_DEBATE_USER_NO FOREIGN KEY (USER_NO)
        REFERENCES USER_INFO (USER_NO) ON DELETE CASCADE;

ALTER TABLE DEBATE
    ADD CONSTRAINT FK_DEBATE_TEAM_NUM FOREIGN KEY (TEAM_NUM)
        REFERENCES TB_TEAM (TEAM_NUM) ON DELETE CASCADE;

-- DEBATE_COMMENT Table Create SQL
CREATE TABLE DEBATE_COMMENT
(
    DEBATE_COMMENT_NUM NUMBER NOT NULL, 
    DEBATE_NUM NUMBER NOT NULL, 
    USER_NO NUMBER NOT NULL,
    DEBATE_COMMENT_CONTENT VARCHAR2(300) NOT NULL,
    DEBATE_COMMENT_DATE DATE DEFAULT SYSDATE NOT NULL,
    CONSTRAINT PK_DEBATE_COMMENT PRIMARY KEY (DEBATE_COMMENT_NUM)
);
COMMENT ON TABLE DEBATE_COMMENT IS '토론 댓글';
COMMENT ON COLUMN DEBATE_COMMENT.DEBATE_COMMENT_NUM IS '토론 댓글 번호';
COMMENT ON COLUMN DEBATE_COMMENT.DEBATE_NUM IS '토론 번호';
COMMENT ON COLUMN DEBATE_COMMENT.USER_NO IS '회원 번호';
COMMENT ON COLUMN DEBATE_COMMENT.DEBATE_COMMENT_CONTENT IS '토론 댓글 내용';
COMMENT ON COLUMN DEBATE_COMMENT.DEBATE_COMMENT_DATE IS '댓글 작성 날짜';

ALTER TABLE DEBATE_COMMENT
    ADD CONSTRAINT FK_DEBATE_COMMENT_DEBATE_NUM FOREIGN KEY (DEBATE_NUM)
        REFERENCES DEBATE (DEBATE_NUM) ON DELETE CASCADE;

ALTER TABLE DEBATE_COMMENT
    ADD CONSTRAINT FK_DEBATE_COMMENT_USER_NO FOREIGN KEY (USER_NO)
        REFERENCES USER_INFO (USER_NO) ON DELETE CASCADE ;

-- 샘플데이터
-- USER_INFO
INSERT INTO USER_INFO VALUES(0, ' ', ' ', ' ', ' ', 0, ' ', ' ', ' ', ' ', ' ', 'N', 'N');
...
VALUES (5, 'user05', 'nick05', '채승민', '$2a$10$1EXm5yZSISCkS1fdcc81b.OnUXtgWQA0PR7eRua7SyuwXIVJRN7Qa', 'user05@test.org', 01056789123, '경기 안산시 단원구 대부황금로 1927', 'music', 'Y', 'N', 31, 'M');
INSERT INTO USER_INFO (USER_NO, USER_ID, USER_NN, USER_NAME, USER_PWD, EMAIL, PHONE, ADDRESS, INTEREST, LOGIN_OK, ADMIN, AGE, GENDER)
VALUES (6, 'user06', 'nick06', '최병철', '$2a$10$VD4nzP.WxXHU5ddfRNt8lu8LvsG9Z0000kAx1PqQSuuEOxvea/iQK', 'user06@test.org', 01067891234, '인천 중구 용유서로302번길 16-15', 'qualification', 'Y', 'N', 29, 'M');
INSERT INTO USER_INFO (USER_NO, USER_ID, USER_NN, USER_NAME, USER_PWD, EMAIL, PHONE, ADDRESS, INTEREST, LOGIN_OK, ADMIN, AGE, GENDER)
VALUES (7, 'user07', 'nick07', '하유진', '$2a$10$k/IP/K8og3uw4WL/Txuou.PkzqViuD5FAIQfhmHuztlkLWiKTGsuC', 'user07@test.org', 01078912345, '인천 중구 차이나타운로59번길 12', 'qualification', 'Y', 'N', 25, 'F');
INSERT INTO USER_INFO (USER_NO, USER_ID, USER_NN, USER_NAME, USER_PWD, EMAIL, PHONE, ADDRESS, INTEREST, LOGIN_OK, ADMIN, AGE, GENDER)
VALUES (8, 'user08', 'nick08', '강예원', '$2a$10$jBKw2piCOy2LGUuhec/aT.3hm.ZpcYU7DaD6X4xIyqiT2BaT2Jjke', 'user08@test.org', 01089123456, '인천 강화군 화도면 해안남로 1481', 'qualification', 'Y', 'N', 22, 'F');
INSERT INTO USER_INFO (USER_NO, USER_ID, USER_NN, USER_NAME, USER_PWD, EMAIL, PHONE, ADDRESS, INTEREST, LOGIN_OK, ADMIN, AGE, GENDER)
VALUES (9, 'user09', 'nick09', '최현우', '$2a$10$N/Tfr8AI9RFG7pMyE46qs.HRif5Q8zXC5b8unXVnBd2HsjEn6GXmW', 'user09@test.org', 01091234567, '인천 계양구 계산동', 'competition', 'Y', 'N', 27, 'M');
INSERT INTO USER_INFO (USER_NO, USER_ID, USER_NN, USER_NAME, USER_PWD, EMAIL, PHONE, ADDRESS, INTEREST, LOGIN_OK, ADMIN, AGE, GENDER)
VALUES (10, 'user10', 'nick10', '김하은', '$2a$10$pI5nOV4JgPo1WYJkNBqKe.ZnOgvTyTvrCMdaiHLQtKYOivjAs4.3S', 'user10@test.org', 01098765432, '인천 연수구 컨벤시아대로 160', 'competition', 'Y', 'N', 24, 'F');
...
VALUES (1, 1, 'ENG 스터디모임', '편하게 영어 문법/회화 공부할 사람들이 모이는 팀입니다.',' ', NULL, NULL,
            '초급', '서울 홍대', '목요일', 48, '오후 4시 이후', NULL, 4, 'Y');
INSERT INTO TB_TEAM (TEAM_NUM, FIELD_NUM, TEAM_NAME, TEAM_INTRO, TEAM_LEADER_INTRO, TEAM_ORIGINAL_IMAGE, 
                            TEAM_RENAME_IMAGE, TEAM_LEVEL, TEAM_LOCAL, TEAM_ACT_DAY,
                            TEAM_ACT_WEEK, TEAM_ACT_TIME, TEAM_FEE, TEAM_LIMIT, TEAM_RECRUIT)
...
INSERT INTO TB_JOIN_WAITING (JOIN_NUM, TEAM_NUM, USER_NO, JOIN_INTRO, JOIN_ORIGINAL_FILENAME, JOIN_RENAME_FILENAME)
VALUES (5, 1, 3, '영어 회화 공부를 하고 싶어, 관련 정보 및 함께 공부할 사람을 찾기 위해 지원합니다. 영어 회화 공부는 약 한달 째 진행 중이며, 수준은 아직 초급입니다.', NULL, NULL);
INSERT INTO TB_JOIN_WAITING (JOIN_NUM, TEAM_NUM, USER_NO, JOIN_INTRO, JOIN_ORIGINAL_FILENAME, JOIN_RENAME_FILENAME)
VALUES (6, 1, 4, '영어 회화 공부를 하고 싶어, 관련 정보 및 함께 공부할 사람을 찾기 위해 지원합니다. 영어 회화 공부는 약 한달 째 진행 중이며, 수준은 아직 초급입니다.', NULL, NULL);

--임수안
--REVIEW
INSERT INTO REVIEW
VALUES(1, 'ENG 스터디모임','영어', '리뷰1', '닉네임1', '쉽고 재밌었습니다.', '4', null, null, default, default);
INSERT INTO REVIEW
VALUES(2, 'SW 스타트업', '프로그래밍', '리뷰2', '닉네임2', '즐거웠습니다.', '5', null, null, default, default);
INSERT INTO REVIEW
VALUES(3, 'ENG 스터디모임', '영어','리뷰3', '닉네임3', '분위기가 좋았습니다.', '5', null, null, default ,default);
INSERT INTO REVIEW
VALUES(4, 'CASTLE 밴드','음악/악기', '리뷰4', 'nick07', '재밌었습니다.', '5', null, null, default,default);
INSERT INTO REVIEW
VALUES(5, 'SQLD 자격증', '자격증', '리뷰6', 'nick08', '즐겁게 배웠습니다.', '3', null, null, default ,default);
INSERT INTO REVIEW
VALUES(6, 'HELLO SEOUL','공모전', '리뷰7', 'nick08', '아쉬웠습니다.', '1', null, null, default ,default);
INSERT INTO REVIEW
VALUES(7, 'ENG 스터디모임','영어','리뷰5', 'nick05', '재밌었습니다.', '4', null, null, default ,default);
INSERT INTO REVIEW
VALUES(8, 'SW 스타트업','프로그래밍', '리뷰8', 'nick10', '괜찮았습니다.', '3', null, null, default ,default);
INSERT INTO REVIEW
VALUES(9, 'SQLD 자격증','자격증', '리뷰9', '닉네임1', '분위기가 좋았습니다.', '5', null, null, default ,default);
INSERT INTO REVIEW
VALUES(10, 'CASTLE 밴드','음악/악기','리뷰10', '닉네임2', '팀장님이 아쉬웠습니다.', '2', null, null, default ,default);
INSERT INTO REVIEW
VALUES(11, 'HELLO SEOUL', '공모전', '리뷰11', '닉네임2', '다들 열심히해서 좋았습니다.', '5', null, null,  default ,default);
INSERT INTO REVIEW
VALUES(12, 'ENG 스터디모임', '영어', '리뷰12', '닉네임1', '괜찮았습니다.', '4', null, null, default ,default);
INSERT INTO REVIEW
VALUES(13, 'SQLD 자격증','자격증','리뷰13', '닉네임1', '소통이 잘 안되서 아쉬웠습니다.', '2', null, null, default ,default);
...
VALUES (0, 0, 0, ' ', ' ', default);
INSERT INTO TB_ALERT
VALUES (1, 1, 1, '팀 정보', '닉네임1 님이 팀 정보 를 변경하였습니다.', DEFAULT);
INSERT INTO TB_ALERT
VALUES (2, 1, 1, '공지', '닉네임1 님이 공지 를 등록하였습니다.', DEFAULT);
INSERT INTO TB_ALERT
VALUES (3, 1, 1, '일지', '닉네임1 님이 일지 를 등록하였습니다.', DEFAULT);
INSERT INTO TB_ALERT
VALUES (4, 1, 1, '일정', '닉네임1 님이 일정 를 등록하였습니다.', DEFAULT);

-- TB_DEBATE SAMPLE DATA
INSERT INTO DEBATE VALUES (0, 0, 0, ' ', ' ', default, default);
INSERT INTO DEBATE VALUES (1, 1, 1, '테스트 샘플1', '테스트 샘플 토론입니다.', default, default);
INSERT INTO DEBATE VALUES (2, 2, 1, '테스트 샘플2', '테스트 샘플 토론입니다.', default, default);
INSERT INTO DEBATE VALUES (3, 1, 1, '테스트 샘플3', '테스트 샘플 토론입니다.', default, default);
INSERT INTO DEBATE VALUES (4, 1, 1, '테스트 샘플4', '테스트 샘플 토론입니다.', default, default);
INSERT INTO DEBATE VALUES (5, 3, 1, '테스트 샘플5', '테스트 샘플 토론입니다.', default, default);
INSERT INTO DEBATE VALUES (6, 1, 1, '테스트 샘플6', '테스트 샘플 토론입니다.', default, default);

-- TB_DEBATE_COMMENT SAMPLE DATA
INSERT INTO DEBATE_COMMENT VALUES (0, 0, 0, ' ', default);
INSERT INTO DEBATE_COMMENT VALUES (1, 4, 2, '테스트 댓글입니다.', default);
INSERT INTO DEBATE_COMMENT VALUES (2, 4, 3, '테스트 댓글입니다.', default);
INSERT INTO DEBATE_COMMENT VALUES (3, 4, 1, '테스트 댓글입니다.', default);
INSERT INTO DEBATE_COMMENT VALUES (4, 6, 3, '테스트 댓글입니다.', default);
INSERT INTO DEBATE_COMMENT VALUES (5, 6, 2, '테스트 댓글입니다.', default);
